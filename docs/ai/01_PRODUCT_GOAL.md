...
4. 初回MVPの代表入力（写真5枚 + memoryText）から、作品として使える4層を生成できる
5. LayerはあとからFrontendで編集・差し替えできる
6. 生成がCloud Run CPU構成で現実的に完了する

初回MVPのArtwork Canvasは入力写真の比率に依存させず、2L判Landscape
（178 mm × 127 mm、`aspectRatio = 178 / 127`）とする。Layer数と写真数のShared Contractは
将来拡張のため可変長を維持する。
