...
def _log_segmentation_timings(
    ...
    *,
    ...
    for stage, elapsed_ms in (
        ...
        ("efficient_sam.encoder_inference", timings.encoder_inference_elapsed_ms),
        ("efficient_sam.decoder_inference", timings.decoder_inference_elapsed_ms),
...
def _semantic_profile_instruction(profile: str) -> str:
    ...
    if profile == "physical_layer_v2":
        return (
            ...
            "least 0.60 of canvas width. Do not set semantic_role or extraction_intent on a "
            "scene_anchor; those are subject-only metadata. Subjects must be self-contained and "
            "must resolve to one "
    ...
    if profile == "physical_layer_v3_architecture":
        return (
            ...
            "A scene_anchor must have exactly one broad rectangular component and must not have "
            "semantic_role or extraction_intent; those are subject-only metadata. Every subject "
            "must "
    ...
    if profile == "coherent_group_planning":
        return (
            ...
            "Use scene_anchor only for one broad rectangular background range. A scene_anchor must "
            "have exactly one component; do not set semantic_role or extraction_intent on a "
            "scene_anchor because "
            "they are subject-only metadata. This profile plans component relationships only; it "
            "does not decide Mask "
