...
def test_build_generator_uses_split_efficientsam_only_when_both_paths_are_configured(
    monkeypatch,
) -> None:
    captured: dict[str, object] = {}

    class FakeSplitSegmenter:
        def __init__(self, encoder_path: Path, decoder_path: Path, max_side: int) -> None:
    ...
    monkeypatch.setattr(generator_service, "LazyEfficientSamSplitOnnxSegmenter", FakeSplitSegmenter)
    monkeypatch.setattr(generator_service, "GeminiArtworkGenerator", FakeGenerator)
    settings = Settings(
        gemini_api_key="test-key",
        gemini_model="gemini-3.5-flash-lite",
        efficientsam_encoder_model_path=Path(".models/efficientsam_ti_encoder.onnx"),
        efficientsam_decoder_model_path=Path(".models/efficientsam_ti_decoder.onnx"),
    )

    generator_service.build_generator(settings)

    assert captured["encoder_path"] == BACKEND_DIR / ".models/efficientsam_ti_encoder.onnx"
    assert captured["decoder_path"] == BACKEND_DIR / ".models/efficientsam_ti_decoder.onnx"
